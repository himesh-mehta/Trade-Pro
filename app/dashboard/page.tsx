    ...
    <motion.div {...fadeInUp} className="border-b border-gray-700">
      <div className="conatiner mx-auto px-4">
        <ul className="flex space-x-8 ">
          {["Stocks", "Mutual Funds", "ETFs", "Optionss"].map((tab) => (
            <motion.li
              className={`py-2 cursor-pointer whitespace-nowrap ${
                activeTab === tab
                  ? "border-b-2 border-blue-500 text-blue-500"
                  : "text-gray-300 hover:text-blue-500 transtion-colors"
              }`}
              key={tab}
...
const Marketindices = () => {
  ...
  const [marketData, setMarketData] = useState([
    { name: "NIFT 50", value: 1834.85, change: 0, percentChange: 0 },
    { name: "BANKNIFTY 50", value: 50256.85, change: 0, percentChange: 0 },
    { name: "SENSEX", value: 30256.85, change: 0, percentChange: 0 },
  ]);
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 my-4">
      {marketData.map((index) => (
        <motion.div
          ...
          whileHover={{ scale: 1.05 }}
          whileTap={{scale:0.95}}
          className="bg-gray-800 p-4 rounded-lg shadow-lg hover:shodow-xl transition-shadow cursor-pointer"
  ...
  return (
    <motion.div {...fadeInUp} className="my-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <StockCard name="Reliance" initialPrice={2346.5} />
        <StockCard name="Tata Motors" initialPrice={21346.5} />
        <StockCard name="Energy" initialPrice={6546.5} />
        <StockCard name="Zomato" initialPrice={656.5} />
      </div>
    </motion.div>
  );
};
